...
export async function POST(req) {
  try {
    ...
    const existingEntry = await db
      .collection("problems")
      .findOne({ problemId: fields.problemId })
    if (existingEntry) {
      return new Response(
        `Unable to insert problem ${fields.problemId} as entry already exists`,
        {
          status: 422,
          statusText: `Problem ID ${fields.problemId} already exists`
        }
      )
    }
    ...
    return new Response(`Document inserted with _id: ${result.insertedId}`, {
      status: 200,
      statusText: `Problem ID ${fields.problemId} inserted into database`
    })
  } catch (e) {
    console.error(e)
