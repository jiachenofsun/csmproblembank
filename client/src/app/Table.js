...
export default function Table({ problems }) {
  ...
  const columns = [
    ...
    columnHelper.accessor("name", {
      ...
      id: "name",
      cell: (info) => (
        <p className="text-center sm:text-left w-full">{info.getValue()}</p>
      )
    }),
    columnHelper.accessor("difficulty", {
      ...
      cell: (info) => (
        <span
          className={`inline-block ${getDifficultyStyles(info.getValue())}`}
        >
          {info.getValue()}
        </span>
      )
    ...
    columnHelper.accessor(
      (row) => (row.resourceLinks.meta.trim() !== "" ? true : false),
      {
        ...
        cell: (info) => (
          <div
            className={`w-5 h-5 rounded-full ${getResourceLinkStyles(info.getValue())}`}
          ></div>
        )
      }
    ),
          ...
          <table className="w-4/5">
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                <tr key={headerGroup.id}>
                  {headerGroup.headers.map((header) => (
                    <th
                      key={header.id}
                      className={`bg-csmGreenDesat border border-gray-400 px-4 py-2 ${getColumnWidth(header.column.columnDef.header)}`}
                    >
                      <>
                        <div>
            ...
            <tbody>
              {table.getRowModel().rows.map((row) => (
                <tr
                  key={row.id}
                  onClick={() => handleRowClick(row.getValue("problemId"))}
                  className="cursor-pointer py-2 hover:bg-gray-200"
                >
                  {row.getVisibleCells().map((cell) => (
                    <td
                      key={cell.id}
                      className={`border border-gray-400 px-4 py-2 ${getColumnWidth(cell.column.columnDef.header)}`}
                    >
  ...
  switch (column.id) {
    ...
    case "slides":
      return (
        <select
          onChange={(event) =>
            column.setFilterValue(JSON.parse(event.target.value))
          }
        >
    ...
    case "topics":
      return (
        <Accordion
          ...
          onClickFn={onClickArrayMemoized}
          column={column}
        />
      )
